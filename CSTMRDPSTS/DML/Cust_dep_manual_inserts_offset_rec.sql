...
--variable declaration
   ...
   V_CREDIT_RED_ROW CUST_DEP_CRD_RED_DETAILS%ROWTYPE;
   V_COMMIT         NUMBER := 0;
   V_CUM_AMT        NUMBER := 0;
   ...
   V_TRAN_SQ_ID     CUSTOMER_DEPOSIT_DETAILS.TRANSACTION_SQ_ID%TYPE;
   V_LOAD_DATE      DATE := SYSDATE;
BEGIN
   FOR REC IN TEMP_CUR LOOP
      BEGIN
         select max(transaction_sq_id)+1 INTO V_TRAN_SQ_ID from CUSTOMER_DEPOSIT_DETAILS;
         ...
         V_TEMP_ROW.TRANSACTION_SQ_ID           :=  V_TRAN_SQ_ID;
         V_TEMP_ROW.NOTES                       := 'Offset of original transaction number';

         TABLE_IU_PKG.CUST_DEP_DET_I_SP(V_TEMP_ROW);--load into CUSTOMER_DEPOSIT_DETAILS table
         
         V_CREDIT_RED_ROW.TRANSACTION_SQ_ID       := V_TEMP_ROW.TRANSACTION_SQ_ID; 
         V_CREDIT_RED_ROW.CRD_RED_SEQ             := CRD_RED_SEQ.NEXTVAL;
         V_CREDIT_RED_ROW.TRANSACTION_TYPE        := V_TEMP_ROW.TRANSACTION_TYPE;
         V_CREDIT_RED_ROW.DEPOSIT_REMAINING_BAL   := V_TEMP_ROW.CSTMR_DPST_SALES_LN_ITM_AMT;
         V_CREDIT_RED_ROW.ORGNL_DEPOSIT_TRANSACTION_NBR  := NULL;
         V_CREDIT_RED_ROW.ORGNL_DEPOSIT_TERMINAL_NBR     := NULL;
         V_CREDIT_RED_ROW.ORGNL_DEPOSIT_TRANSACTION_DATE := NULL;
         IF V_TEMP_ROW.CSTMR_DPST_SALES_LN_ITM_AMT < 0 THEN
            CUSTOMER_DEPOSIT_MAINT_PKG.ORIG_DEP_REM_BAL_UPD( V_TEMP_ROW.CSTMR_DPST_SALES_LN_ITM_AMT, REC.CUSTOMER_ACCOUNT_NUMBER, V_TEMP_ROW.TRANSACTION_DATE, V_ORIG_DEP_NBR, V_ORIG_TERM_NBR, V_ORIG_TRAN_DATE);
            V_CREDIT_RED_ROW.DEPOSIT_REMAINING_BAL  := NULL;
            V_CREDIT_RED_ROW.ORGNL_DEPOSIT_TRANSACTION_NBR  := V_ORIG_DEP_NBR;  
            V_CREDIT_RED_ROW.ORGNL_DEPOSIT_TERMINAL_NBR     := V_ORIG_TERM_NBR;
            V_CREDIT_RED_ROW.ORGNL_DEPOSIT_TRANSACTION_DATE := V_ORIG_TRAN_DATE;
         END IF; 
         
         TABLE_IU_PKG.CUST_DEPOSIT_CRD_RED_I_SP(V_CREDIT_RED_ROW);
         V_TEMP_ROW := NULL;
         V_COMMIT := V_COMMIT + 1;
