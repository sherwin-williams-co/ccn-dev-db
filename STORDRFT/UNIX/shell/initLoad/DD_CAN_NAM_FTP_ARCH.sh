...
mv "DLY_DRAFT_CAN_NAM" $HOME/initLoad/STORDRFT/archieve/Daily_bank/DLY_DRAFT_CAN_NAM"_"$date
echo "DLY_DRAFT_CAN_NAM has been archieved to $HOME/initLoad/STORDRFT/archieve/Daily_bank/ path"

#################################################
#    ERROR STATUS CHECK DD_CAN_NAM_FTP_ARCH.sh 
#################################################
status=$?
if test $status -ne 0
   then
     echo "processing FAILED for DD_CAN_NAM_FTP_ARCH at ${TIME} on ${DATE}"
     exit 1;
fi
