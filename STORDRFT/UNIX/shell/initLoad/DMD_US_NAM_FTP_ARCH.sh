...
#put DLY_MAINT_DRAFT_US_NAM 'SMIS1.STBD1361(+1)'
#
#bye
#END_SCRIPT
...
mv "DLY_MAINT_DRAFT_US_NAM" $HOME/initLoad/STORDRFT/archieve/Maintenance/DLY_MAINT_DRAFT_US_NAM"_"$date
echo "DLY_MAINT_DRAFT_US_NAM has been archieved to $HOME/initLoad/STORDRFT/archieve/Maintenance/ path"

#################################################
#    ERROR STATUS CHECK DMD_US_NAM_FTP_ARCH.sh 
#################################################
status=$?
if test $status -ne 0
   then
     echo "processing FAILED for DMD_US_NAM_FTP_ARCH at ${TIME} on ${DATE}"
     exit 1;
fi
