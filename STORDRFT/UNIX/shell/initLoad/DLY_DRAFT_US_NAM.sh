...
TIME=`date +"%H:%M:%S"`
DATE=`date +"%m/%d/%Y"`
echo "START SD_BANKFILES_PKG.CREATE_US_NONAUTO_FILE Query : Processing Started at $TIME on $DATE "

./EXEC_PROC_1PARAM.sh "SD_BANKFILES_PKG.CREATE_US_NONAUTO_FILE" "$P1"

#################################################
#    ERROR STATUS CHECK DLY_DRAFT_US_NAM.sh 
#################################################
status=$?
if test $status -ne 0
   then
     echo "processing FAILED for DLY_DRAFT_US_NAM at ${TIME} on ${DATE}"
     exit 1;
fi

TIME=`date +"%H:%M:%S"`
echo "END SD_BANKFILES_PKG.CREATE_US_NONAUTO_FILE Query : Processing finished at $TIME on $DATE "  
