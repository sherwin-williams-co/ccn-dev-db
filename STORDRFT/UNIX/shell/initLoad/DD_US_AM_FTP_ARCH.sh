...
mv "DLY_DRAFT_US_AM" $HOME/initLoad/STORDRFT/archieve/Daily_bank/DLY_DRAFT_US_AM"_"$date
echo "DLY_DRAFT_US_AM has been archieved to $HOME/initLoad/STORDRFT/archieve/Daily_bank/ path"

#################################################
#    ERROR STATUS CHECK DD_US_AM_FTP_ARCH.sh 
#################################################
status=$?
if test $status -ne 0
   then
