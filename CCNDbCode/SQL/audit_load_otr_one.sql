...
CURSOR SEL_AUDIT_LOG IS
       ...
       (SELECT *
         ...
         WHERE AUDIT_REC_FLAG IS NULL
           ...
           AND TRANSACTION_ID IN ('|0347042|', '|0374397|', '|0336968|')
ORDER BY 2, 3 DESC) ABC
WHERE ABC.TRANSACTION_DATE = (SELECT MIN(TRANSACTION_DATE) FROM AUDIT_LOG WHERE TRANSACTION_ID = ABC.TRANSACTION_ID);
...
BEGIN
    ...
    SELECT TABLE_ROW_DATA, LOG_ID, TABLE_PK_VALUE INTO ROW_CURR, V_LOG_ID_CURR, PK_CURR -- Added to accomodate population of primary key columns in backFeed files generated
     ...
     WHERE TABLE_NAME        = IN_TABLE_NAME
       AND AL.TRANSACTION_ID = IN_KEY
       AND AL.TRANSACTION_DATE IN (SELECT MIN(AL1.TRANSACTION_DATE)
                                     FROM AUDIT_LOG AL1
                                    WHERE AL1.TABLE_NAME     = AL.TABLE_NAME
                                      AND AL1.TRANSACTION_ID = IN_KEY)
       AND AL.LOG_ID = (SELECT MIN( AL2.LOG_ID)
                          FROM	AUDIT_LOG AL2
