...
SELECT PCC_PCL_STORE

  INTO V_PCC_PCL_STORE

  FROM COST_CENTER

 WHERE COST_CENTER_CODE = rec.COST_CENTER_CODE;

--DBMS_OUTPUT.PUT_LINE(rec.COST_CENTER_CODE);

DBMS_OUTPUT.PUT_LINE(V_PCC_PCL_STORE.GETCLOBVAL());

    V_VAL := '<?xmlversion="1.0"encoding="UTF-8"?><PCC_PCL_STR>';


 


    IF rec.PCL IS NOT NULL THEN

        V_VAL :=  V_VAL ||  '<PCL><CC>'||rec.PCL_COST_CENTER_CODE||'</CC></PCL>';

    END IF;

    IF rec.PCC IS NOT NULL THEN

        V_VAL :=  V_VAL ||  '<PCC><CC>'||rec.PCC_COST_CENTER_CODE||'</CC></PCC>';

    END IF;

V_VAL :=  V_VAL || '</PCC_PCL_STR>';
