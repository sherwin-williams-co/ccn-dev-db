...
FOR REC IN (SELECT * FROM TEMP_CC_SEG_TERR_NAME) LOOP
    BEGIN
     ...
     EXCEPTION
         WHEN OTHERS THEN
             DBMS_OUTPUT.PUT_LINE('FAILED TO UPDATE BUSN_CODE FOR COST CENTER: ' ||REC.COST_CENTER_CODE);
     END;
     BEGIN
         UPDATE COST_CENTER
     ...
     EXCEPTION
         WHEN OTHERS THEN
             DBMS_OUTPUT.PUT_LINE('FAILED TO UPDATE COST CENTER NAME FOR COST CENTER: ' ||REC.COST_CENTER_CODE);
     END;
     V_COUNT := V_COUNT +1;
     IF V_COUNT = 100 THEN
	      COMMIT;
