...
DECLARE
	CURSOR UPD_COST_CENTER IS SELECT * FROM COST_CENTER A WHERE COST_CENTER_CODE LIKE '%T' AND COST_CENTER_CODE NOT IN ('83MXMT', '83VIET', '81GPOT','OOPPFT', '76782T')
...
DECLARE
    CURSOR INS_DISPATCH_TERMINAL IS
		SELECT  COST_CENTER_CODE,
			    COST_CENTER_NAME,
			    CATEGORY
				FROM COST_CENTER A
				WHERE A.CATEGORY = 'D'
		            AND NOT EXISTS(SELECT 'X' FROM DISPATCH_TERMINAL D WHERE A.COST_CENTER_CODE = D.COST_CENTER_CODE) ;
...
BEGIN
	FOR REC IN INS_DISPATCH_TERMINAL LOOP
		I := I +1;
		FOR J IN HOME_STORE(REC.COST_CENTER_NAME) LOOP
		    V_HOME_STORE := J.COST_CENTER_CODE;
		END LOOP;
		INSERT INTO DISPATCH_TERMINAL SELECT REC.COST_CENTER_CODE, REC.CATEGORY, V_HOME_STORE FROM DUAL;
		IF I > 100 THEN
			I := 0;
			COMMIT;
		END IF;
