...
CURSOR SEL_AUDIT_LOG IS
     SELECT * 
       FROM AUDIT_LOG 
      WHERE TABLE_NAME IN ('TERRITORY_EMPLOYEE','MANAGER_EMPLOYEE')
        AND UPPER(CCN_AUDIT_PKG.XML_EXTRACT_NO_EXCEPTION(TABLE_ROW_DATA,'//EMP_PAYROLL_STATUS')) = 'TERM'
        AND TRUNC(TRANSACTION_DATE) BETWEEN TRUNC(TO_DATE('11-AUG-2015')) AND TRUNC(TO_DATE('05-NOV-2015'))
        AND AUDIT_REC_EFF_DATE IS NULL
...
V_EMPL_ID              TERRITORY_EMPLOYEE.GEMS_ID_NUMBER%TYPE;
...
V_CURR_TRANS_DATE     AUDIT_LOG.TRANSACTION_DATE%TYPE;
...
BEGIN
    ...
    SELECT TABLE_ROW_DATA, LOG_ID, TABLE_PK_VALUE, TRANSACTION_DATE  
      INTO ROW_CURR, V_LOG_ID_CURR, PK_CURR, V_CURR_TRANS_DATE -- Added to accomodate population of primary key columns in backFeed files generated
      FROM AUDIT_LOG AL
     WHERE TABLE_NAME        IN ('TERRITORY_EMPLOYEE', 'MANAGER_EMPLOYEE')
       AND AL.TRANSACTION_ID = IN_KEY
       AND UPPER(CCN_AUDIT_PKG.XML_EXTRACT_NO_EXCEPTION(AL.TABLE_ROW_DATA,'//EMP_PAYROLL_STATUS')) = 'TERM'
       AND TRUNC(AL.TRANSACTION_DATE) BETWEEN TRUNC(TO_DATE('11-AUG-2015')) AND TRUNC(TO_DATE('05-NOV-2015'))
    ...
    BEGIN
        SELECT TABLE_ROW_DATA, LOG_ID 
          INTO ROW_PREV, V_LOG_ID_PREV
          FROM AUDIT_LOG AL
         WHERE AL.TABLE_NAME       IN ('TERRITORY_EMPLOYEE', 'MANAGER_EMPLOYEE')
           AND AL.TRANSACTION_ID    = IN_KEY
           AND AL.TRANSACTION_DATE IN (SELECT MAX(AL1.TRANSACTION_DATE)
                                         FROM AUDIT_LOG AL1
                                        WHERE AL1.TABLE_NAME     = AL.TABLE_NAME
                                          AND AL1.TRANSACTION_ID = IN_KEY
                                          AND AL1.TRANSACTION_DATE < TRUNC(TO_DATE(V_CURR_TRANS_DATE)))
           AND ROWNUM < 2
