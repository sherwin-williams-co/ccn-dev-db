...
####################################################################################################################################
# Script Name    :  MAIN_AUDIT_OTR_THREE (Control-M)
#
...
# Created         :  AXK326 12/04/2015 CCN Project Team....
###################################################################################################################################
...
############################################################################
#         execute audit_load_otr_three.sh shell to create CCN Backfeed File
############################################################################
echo "Processing Started for audit_load_otr_three at ${TIME} on ${DATE}"
./audit_load_otr_three.sh
#############################################
#   ERROR STATUS CHECK  audit_load_otr_three shell
#############################################
status=$?
if test $status -ne 0 
   then
     echo "processing FAILED for audit_load_otr_three at ${TIME} on ${DATE}"
     exit 1;
fi
TIME=`date +"%H:%M:%S"`
echo "Processing Finished for audit_load_otr_three at ${TIME} on ${DATE}"
