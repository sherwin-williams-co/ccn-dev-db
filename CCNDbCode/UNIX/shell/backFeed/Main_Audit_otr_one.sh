...
####################################################################################################################################
# Script Name    :  MAIN_AUDIT_OTR_ONE (Control-M)
#
...
#    
# audit_load_otr_one.sh   -->  Creates the Audit Back Feed File 
...
############################################################################
#         execute audit_load_otr_one.sh shell to create CCN Backfeed File
############################################################################
echo "Processing Started for audit_load_otr_one at ${TIME} on ${DATE}"
./audit_load_otr_one.sh
#############################################
#   ERROR STATUS CHECK  audit_load_otr_one shell
#############################################
status=$?
if test $status -ne 0 
   then
     echo "processing FAILED for audit_load_otr_one at ${TIME} on ${DATE}"
     exit 1;
fi
TIME=`date +"%H:%M:%S"`
echo "Processing Finished for audit_load_otr_one at ${TIME} on ${DATE}"
