...
for files in "$DATADIR/$GL_PS_FILE_NAME"*".POLLINGDONE"
do
    ...
    cd "$CLASSHOME" || exit
    REQUESTID=$(java com.webservice.PollingRequest "$PRIMESUBUSERNAME" "$PRIMESUBPASSWORD" "$PRIMESUBENVIRONMENT" "$FILENAME" "$ENVIRON" "$PREV_RQST_ID") 
