#!/bin/sh
############################################################################
# Script name : ter_upd_pos_rqst_id.sh
...
PROC_NAME="ter_upd_pos_rqst_id.sh"
...
for files in "$DATADIR/$TER_FILE_NAME"*".REQUEST"
do
...
then
    ...
    ./ter_exec_pos_downloads_update.sh "$FILENAME" "$REQUESTID" 	
    status=$?
    TIME=$(date +"%H%M%S")
