...
#
# Date Created: 04/28/2014 JXC517
# Date Updated: 
#
##########################################################################################

echo "\n begin adhoc_polling_status_update_1.sh script"
...
DATE=`date +"%Y-%m-%d"`
TIME=`date +"%H:%M:%S"`
PROC="adhoc_polling_status_update_1.sh"
...
BEGIN
    ...
    FOR rec IN (SELECT * FROM COST_CENTER_POLLING WHERE POLLING_STATUS IS NOT NULL) LOOP
        ...
        FOR bankcard_rec IN (SELECT *
            ...
            V_XML := V_XML || BUILD_TAG(rec.POLLING_STATUS,'POLLING_STATUS_CODE');
            V_XML := V_XML || BUILD_TAG(NVL(rec.MERCHANT_ID,bankcard_rec.MERCHANT_ID),'MERCHANT_ID');
            V_XML := V_XML || BUILD_TAG(NVL(rec.OPEN_DATE,TO_CHAR(bankcard_rec.EFFECTIVE_DATE, 'RRRRMMDD')),'EFFECTIVE_DATE','Y');
            ...
            V_XML := V_XML || BUILD_TAG(NVL(rec.AMEX_ID,bankcard_rec.AMEX_SE_ID),'AMEX_SE_ID');
            V_XML := V_XML || BUILD_TAG(NVL(rec.DISCOVER_ID,bankcard_rec.DISCOVER_ID),'DISCOVER_ID');
            V_XML := V_XML || '</BANK_CARD>';
        END LOOP;
...
echo "\n ending adhoc_polling_status_update_1.sh script \n"
