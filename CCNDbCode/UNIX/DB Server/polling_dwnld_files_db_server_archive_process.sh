#!/bin/sh
###############################################################################################################################
# Script name   : polling_dwnld_files_db_server_archive_process.sh
...
. /app/ccn/host.sh
PROC_NAME=polling_dwnld_files_db_server_archive_process.sh
...
if [ "$status" -ne 0 ]
then
    $HOME/send_mail.sh "ARCHIVINGPROCESSFAILURE" "Error in $PROC_NAME while archiving files $FILENAME and $DONEFILENAME"
