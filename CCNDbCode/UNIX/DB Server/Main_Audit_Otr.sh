...
####################################################################################################################################
# Script Name    :  MAIN_AUDIT_OTR (Control-M)
#
...
#    
# audit_load_otr.sh   -->  Creates the Audit Back Feed File 
...
############################################################################
#         execute audit_load_otr.sh shell to create CCN Backfeed File
############################################################################
echo "Processing Started for audit_load_otr at ${TIME} on ${DATE}"
./audit_load_otr.sh
#############################################
#   ERROR STATUS CHECK  audit_load_otr shell
#############################################
status=$?
if test $status -ne 0 
   then
     echo "processing FAILED for audit_load_otr at ${TIME} on ${DATE}"
     exit 1;
fi
TIME=`date +"%H:%M:%S"`
echo "Processing Finished for audit_load_otr at ${TIME} on ${DATE}"
