...
CASE WHEN C.STD_COST_IDENTIFIER  IS NULL 
AND C.STATEMENT_TYPE IN ('CN','AC') 
AND NVL(COMMON_TOOLS.IS_CC_SELLING_STORE(C.COST_CENTER_CODE), 'N')= 'Y' 
THEN '08'
ELSE C.STD_COST_IDENTIFIER END AS STD_COST_IDENTIFIER,
CASE WHEN C.PRIM_COST_IDENTIFIER IS NULL
AND c.STATEMENT_TYPE IN ('CN','AC') 
AND NVL(COMMON_TOOLS.IS_CC_SELLING_STORE(c.COST_CENTER_CODE), 'N') = 'Y' 
THEN '08'
