...
Modified : 02/17/2015 SXT410 Added FAX_PHONE_NUMBER, POLLING_STATUS_CODE and
         ...
         : 03/14/2018 nxk927 Added POLLING_START_DATE, POLLING_STOP_DATE
********************************************************************************/  
C.COST_CENTER_CODE,
COST_CENTER_NAME,
C.CATEGORY,
...
(SELECT POLLING_START_DATE
  ...
  WHERE CURRENT_FLAG = 'Y'
    ...
    AND EXPIRATION_DATE IS NULL) POLLING_START_DATE,
(SELECT POLLING_STOP_DATE
   FROM POLLING
  WHERE CURRENT_FLAG = 'Y'
    AND COST_CENTER_CODE = C.COST_CENTER_CODE
    AND EXPIRATION_DATE IS NULL) POLLING_STOP_DATE,
(SELECT HOME_STORE 
   FROM TERRITORY
  WHERE CATEGORY = 'T'
