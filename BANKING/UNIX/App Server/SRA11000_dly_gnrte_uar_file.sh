#!/bin/sh
#################################################################
# Script name   : SRA11000_dly_gnrte_uar_file.sh
#
...
proc_name="SRA11000_dly_gnrte_uar_file"
LOGDIR=$HOME/logs
DATE=`date +"%m/%d/%Y"`
TIME=`date +"%H:%M:%S"`
...
#################################################################
echo "Processing started for STR_BNK_DPST_DLY_RCNCL_PROCESS.GNRTE_UAR_FILE at ${TIME} on ${DATE}"
...
BEGIN
:exitCode := 0;
STR_BNK_DPST_DLY_RCNCL_PROCESS.GNRTE_UAR_FILE('$LOAD_DATE');
Exception
 when others then
 :exitCode := 2;
 END;
...
then
    TIME=`date +"%H:%M:%S"`
    echo "processing FAILED for STR_BNK_DPST_DLY_RCNCL_PROCESS.GNRTE_UAR_FILE at ${TIME} on ${DATE}"
    exit 1;
fi
